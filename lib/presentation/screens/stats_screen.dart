...
class _StatsScreenState extends State<StatsScreen> {
  ...
  void initState() {
    ...
    _entriesStream = context.read<StatsRepository>().watchAllStatEntries();
    _pendingStatsGamesStream = context
        .read<GameRepository>()
        .watchPendingStatsGames();
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (isGuest) {
      return _GuestStatsBody(
        players: context.read<PlayerRepository>(),
    ...
    final applyGameStats = context.read<ApplyGameStatsUseCase>();
                          ...
                          children: [
                            for (final game in pending)
                              _PendingStatsCard(
                                ...
                                onAdd: () => applyGameStats(game),
                                onIgnore: () =>
                                    gameRepo.ignoreGameStats(game.id),
                              ),
                          ],
                        );
                      },
                  ...
                  if (!isAdmin)
                    Card(
                      child: Padding(
                        padding: const EdgeInsets.all(12),
                        child: Text(
                          'Sesión actual: ${context.read<AuthController>().currentUser?.email ?? '(sin correo)'} '
...
class _GuestStatsBody extends StatefulWidget {
  ...
  const _GuestStatsBody({required this.players, required this.games});
...
class _GuestStatsBodyState extends State<_GuestStatsBody> {
  ...
  void initState() {
    super.initState();
    _playersStream = widget.players.watchAllPlayers();
    _gamesStream = widget.games.watchFinishedGames();
  }
