...
class _ActiveGameScreenState extends State<ActiveGameScreen> {
  ...
  void initState() {
    super.initState();
    _playersStream = context.read<PlayerRepository>().watchAllPlayers();
  }

  @override
  Widget build(BuildContext context) {
    final games = context.read<GameRepository>();
    final addRound = context.read<AddRoundUseCase>();
    final updateRound = context.read<UpdateRoundUseCase>();
    final deleteRound = context.read<DeleteRoundUseCase>();
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => _confirmCancelGame(context, games),
          ),
        ],
      ),
      body: StreamBuilder<Game?>(
        stream: games.watchGame(widget.gameId),
                    ...
                    Expanded(
                      child: Card(
                        ...
                        child: StreamBuilder<List<Round>>(
                          stream: games.watchRounds(widget.gameId),
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        OutlinedButton(
                          ...
                          onPressed: () => _confirmReset(context, games),
                          child: const Text(
                            'REINICIAR',
                            style: TextStyle(
                              fontFamily: 'Poppins',
  ...
  void _confirmDeleteRound(
    ...
    DeleteRoundUseCase deleteRound,
    Round round,
  ) {
    showDialog(
      context: context,
      builder: (dialogContext) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(
            ...
            onPressed: () {
              deleteRound(widget.gameId, round.id);
    ...
    showDialog(
      ...
      builder: (dialogContext) => _QuickAddRoundDialog(
        ...
        onSubmit: (value, bothTeams) {
          ...
          addRound(widget.gameId, teamAPoints, teamBPoints);
        },
        onRename: (newLabel) =>
            games.renameGameTeam(widget.gameId, team, newLabel),
      ),
    );
  }

  void _confirmReset(BuildContext context, GameRepository games) {
    showDialog(
      context: context,
      builder: (dialogContext) => AlertDialog(
  ...
  void _confirmCancelGame(BuildContext context, GameRepository games) {
    showDialog(
      context: context,
      builder: (dialogContext) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(
            ...
            onPressed: () {
              games.cancelGame(widget.gameId);
              Navigator.pop(dialogContext);
              Navigator.pop(context);
            },
