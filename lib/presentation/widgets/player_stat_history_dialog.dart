...
Future<void> _addEntries(
  ...
  try {
    await stats.addPlayerStatEntries(
      playerId,
      isWin,
      count,
...
Future<void> _editEntry(
  ...
  if (action == 'delete') {
    try {
      await stats.deletePlayerStatEntry(playerId, entry.id);
      if (!context.mounted) return;
  ...
  if (action == 'date') {
    ...
    try {
      await stats.updatePlayerStatEntryDate(playerId, entry.id, updated);
      if (!context.mounted) return;
    ...
    builder: (dialogContext) => StatefulBuilder(
      builder: (dialogContext, setState) {
        Future<void> deleteSelected() async {
          ...
          try {
            await stats.deletePlayerStatEntries(
              player.id,
              selectedIds.toList(),
            );
            setState(() {
              ...
              children: [
                ...
                Expanded(
                  child: StreamBuilder<List<PlayerStatEntry>>(
                    stream: stats.watchPlayerStatEntries(player.id),
